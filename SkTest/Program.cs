...
namespace SkTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Array.FindAll(split, x => rgx.IsMatch(x.Option))
            //  int index = Array.FindIndex(split, x => x.IndexOf("{ 접미어 속성 부여") == 0);

        }
        public static void SearchAndBrowser()
        {
            ...
            string[] asData = (s ?? "").Trim().Split(new string[] { "--------" }, StringSplitOptions.None);
            CurrentItemPrice currentItemPrice = new CurrentItemPrice();
            string option = asData[5];
            option = option.Replace("\r\n", "\n");
            string[] optionData = option.Split('\n');
            foreach (string input in optionData)
            {
                OptionParser.test(currentItemPrice, input, searchItems);
            }
            if (currentItemPrice.prefix == null && currentItemPrice.surfix == null)
            {
                return;
            }
            if (currentItemPrice.prefix == null)
            {
                currentItemPrice.prefix = searchItems.PrefixList.Last();

                currentItemPrice.searchPrefixFilter = new q_Stats_filters()
                {
                    Disabled = false,
                    Id = currentItemPrice.prefix.filter.Id,
                    Value = (new q_Min_And_Max() { Min = 99999, Max = 99999 })
                };

            }
            if (currentItemPrice.surfix == null)
            {
                currentItemPrice.surfix = searchItems.SuffixList.Last();
                currentItemPrice.searchSurfixFilter = new q_Stats_filters()
                {
                    Disabled = false,
                    Id = currentItemPrice.surfix.filter.Id,
                    Value = (new q_Min_And_Max() { Min = 99999, Max = 99999 })
                };
            }
            OpenWebBrowser(currentItemPrice);
        }
        ...
        public static void SearchCurrentItem()
        {
            ...
            Item item = Array.FindAll(searchItems.ItemList.ToArray(), x => itemName.Equals(x.Name)).First();
        ...
        public static void OpenWebBrowser(CurrentItemPrice currentItemPrice)
        {
            ...
            bool search)
        {
            if (prefix.IsRange)
            {
                if (suffix.IsRange)
                {
                    prices.Add(SearchItem.GetPrice(item, prefix, suffix, search, prefix.Min, suffix.Min));
                  //  prices.Add(SearchItem.GetPrice(item, prefix, suffix, prefix.Max, suffix.Max));
                }
                else
                {
                    prices.Add(SearchItem.GetPrice(item, prefix, suffix, search, prefix.Min));
            ...
            else
            {
                if (suffix.IsRange)
                {
                    prices.Add(SearchItem.GetPrice(item, prefix, suffix, search, 99999, suffix.Min));
                    //prices.Add(SearchItem.GetPrice(item, prefix, suffix, 99999, suffix.Max));
                }
                else
                {
                    prices.Add(SearchItem.GetPrice(item, prefix, suffix, search));
                }
                /*
                if (suffix.IsRange)
                {
